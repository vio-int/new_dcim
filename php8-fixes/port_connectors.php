<?php
	...
	if(isset($_POST["action"])&&(($_POST["action"]=="Create")||($_POST["action"]=="Update"))){
		$mfg->PortID=$_POST["PortID"];
		$mfg->Connector=trim($_POST["name"]);
		
		if($mfg->Connector != null && $mfg->Connector != ""){
			if($_POST["action"]=="Create"){
				if($mfg->CreateObject()){
					header('Location: '.redirect("port_connectors.php?PortID=$mfg->PortID"));
				}else{
	...
	$mfgList=$mfg->GetPortConnectorsList();
  ...
  <script type="text/javascript">
	$(document).ready(function() {
		...
		$('#PortID').change(function(e){
			location.href='port_connectors.php?PortID='+this.value;
		});
...
<div class="form-group">
   <label for="PortID">',__("Connector Name"),'</label>
   <input type="hidden" name="action" value="query">
   <select name="PortID" id="PortID" class="form-control">
   <option value=0>',__("New Connector"),'</option>';
...
<div class="form-group">
   ...
   <input type="text" class="form-control validate[required,minSize[1],maxSize[40]]" name="name" id="name" maxlength="40" value="',$mfg->Connector,'">
</div>

<div class="caption">';

	if($mfg->PortID >0){
		echo '   <button class="btn btn-primary" type="submit" name="action" value="Update">',__("Update"),'</button>
