<?php
	...
	if(isset($_POST["action"])&&(($_POST["action"]=="Create")||($_POST["action"]=="Update"))){
            ...
            $mfg->Device_a=trim($_POST["device_a"]);
            $mfg->Interface_a=trim($_POST["interface_a"]);
            $mfg->Device_b=trim($_POST["device_b"]);
            $mfg->Interface_b=trim($_POST["interface_b"]);
            
            if($mfg->Name != null && $mfg->Name != ""){
                    if($_POST["action"]=="Create"){
                            if($mfg->CreateObject()){
                                    header('Location: '.redirect("interface_conn.php?PortID=$mfg->PortID"));
                            }else{
	...
	$mfgList=$mfg->GetInterfaceConnList();
  ...
  <script type="text/javascript">
	$(document).ready(function() {
            ...
            $('#PortID').change(function(e){
                    location.href='interface_conn.php?PortID='+this.value;
            });
    ...
    <div class="row">
        <div class="col-sm-12 col-md-12">
            <ol class="breadcrumb">
                ...
                <li><a href="interface_conn_list.php">Interface Connection</a></li>
                <li><?php echo $mfg->PortID!=""?'Edit':'Add';?> Interface Connection</li>
            </ol>
        </div>
    </div>
    <!-- Breadcrumb code end -->
...
<div class="panel panel-default">
    <div class="panel-heading"><strong>Interface Connection</strong></div>
    ...
    <div class="form-group">
       <label class="col-sm-3" for="device_a">',__("Device A"),'</label>
       <div class="col-sm-9">        
       <select name="device_a" id="device_a" class="form-control">
       <option value="">',__("-- Select --"),'</option>';
            foreach($DeviceList as $mfgRow){
                    if($mfg->Device_a==$mfgRow->PortID){$selected=" selected";}else{$selected="";}
                    echo "<option value=\"$mfgRow->PortID\"$selected>$mfgRow->Name</option>\n";
            }
    echo '</select></div>
    </div>
    <div class="form-group">
       <label class="col-sm-3" for="interface_a">',__("Interface A"),'</label>
       <div class="col-sm-9">
       <input type="text" class="form-control" name="interface_a" id="interface_a" value="',$mfg->Interface_a,'">
       </div>    
    </div>
    <div class="form-group">
       <label class="col-sm-3" for="device_b">',__("Device B"),'</label>
       <div class="col-sm-9">        
       <select name="device_b" id="device_b" class="form-control">
       <option value="">',__("-- Select --"),'</option>';
            foreach($DeviceList as $mfgRow){
                    if($mfg->Device_b==$mfgRow->PortID){$selected=" selected";}else{$selected="";}
                    echo "<option value=\"$mfgRow->PortID\"$selected>$mfgRow->Name</option>\n";
            }
    echo '</select></div>
    </div>
    <div class="form-group">
       <label class="col-sm-3" for="interface_b">',__("Interface B"),'</label>
       <div class="col-sm-9">
       <input type="text" class="form-control" name="interface_b" id="interface_b" value="',$mfg->Interface_b,'">
       </div>    
    </div>
</div>
</div>
<div class="text-center">';
    ...
    echo '&nbsp;<a href="interface_conn_list.php" class="btn-panel btn-success">',__("Cancel"),'</a>';
?>
</div>
    </div>
</div>
