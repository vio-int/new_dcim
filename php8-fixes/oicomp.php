<?php
	...
	if(isset($_POST["action"])&&(($_POST["action"]=="Create")||($_POST["action"]=="Update"))){
            ...
            $mfg->Outer_inter=trim($_POST["outer_inter"]);

            if($mfg->Name != null && $mfg->Name != ""){
                    if($_POST["action"]=="Create"){
                            if($mfg->CreateObject()){
                                    header('Location: '.redirect("oicomp.php?PortID=$mfg->PortID"));
                            }else{
	...
	$mfgList=$mfg->GetOiCompList();
        $mfgConnectorList = $mfg->GetOInterfaceList();
  ...
  <script type="text/javascript">
	$(document).ready(function() {
            ...
            $('#PortID').change(function(e){
                    location.href='oicomp.php?PortID='+this.value;
            });
...
<div class="form-group">
   <label for="PortID">',__("Outer Interface Compatibility"),'</label>
   <input type="hidden" name="action" value="query"><select name="PortID" id="PortID" class="form-control">
   <option value=0>',__("New Compatibility"),'</option>';
...
<div class="form-group">
   <label for="Outer Interface">',__("Outer Interface"),'</label>
   <input type="hidden" name="action" value="query"><select name="outer_inter" id="outer_inter" class="form-control">
   <option value=0>',__("-- Outer Interface --"),'</option>';

	foreach($mfgConnectorList as $mfgRow){
            if($mfg->Inter_name==$mfgRow->PortID){$selected=" selected";}else{$selected="";}
            echo "<option value=\"$mfgRow->PortID\"$selected>$mfgRow->Inter_name</option>\n";
	}
