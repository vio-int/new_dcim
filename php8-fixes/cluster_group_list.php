...
$ClusterGroup_arr = $ClusterGroup->GetClusterGroupListRows($filter);
$ClusterGroup_res = json_decode(json_encode($ClusterGroup_arr), true);


/* echo "<pre>";
print_r($ClusterGroup_res);
echo "<pre/>";exit; */
...
if(isset($_POST['action']) && $_POST["action"]=="Delete"){
    ...
    if(isset($_POST["TransferTo"])){
        $ClusterGroup->PortID=$_POST['PortID'];
        if($ClusterGroup->DeleteObject($_POST["TransferTo"])){
            $response=true;
        }
    }
...
if(isset($_GET['export'])){
    $response = $ClusterGroup->ExportReport($filter);
    exit;
}
// Export Code End
?>
    ...
    <!-- LISTING CODE START -->
        <div class="container wrapper">
            ...
            <div class="row">
                ...
                <div class="col-sm-3">
                    <a href="cluster_group.php" class="btn btn-primary">
                ...
                <div class="col-md-12">
                    <div class="table-responsive">
                            <table class="table table-hover table-headings table-striped">
                                <thead>
                                    <tr>
                                        <th class="orderable"><a href="?sort=v.name&sort_by=<?= $sort_by ?>">Group Name</a></th>
                                ...
                                <tbody>
                                    <?php if(count($ClusterGroup_res) > 0){
                                        foreach($ClusterGroup_res as $res) { ?>
                                    <tr>
                                        <td><?php echo '<a href="cluster_list.php?group='.$res['PortID'].'">'.$res['Name'].'</a>'; ?></td>
                                        <td><?php echo $res['Slug'] ?></td>
                                        <td><?php echo $res['Total_cluster']!=""?$res['Total_cluster']:"0"; ?></td>
                                        <td><a href="cluster_group.php?PortID=<?php echo $res['PortID'] ?>"><i class="fa fa-edit"></i></a>
                                ...
                                <tfoot>
                                    <tr><td colspan="4" class="text-right">
                                        <?php 
                                            ...
                                            $total_rec = $ClusterGroup->GetDashClusterGroupList(); 
