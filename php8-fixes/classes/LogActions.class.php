...
//		$this->UserID=(ArraySearchRecursive($this->UserID,$p->GetUserList(),'UserID'))?$this->UserID:'';
	...
	function Search($num_rec_per_page=0,$page=1){
        ...
        // Function for get specific class result
        function GetClassLog($filter){
            $limit = 50; 
            ...
            {
                $incr .= " AND Class='".$filter['class']."'";
            }
            if(isset($filter['object']))
            {
                $incr .= " AND ObjectID=".$filter['object'];
            }
		
            $sql="SELECT * FROM fac_GenericLog WHERE UserID!='' {$incr} ORDER BY {$sort_on} {$sort_by} LIMIT {$start_from} , {$limit};";
	...
	static function GetDashClassLog($filter){
            global $dbh;
