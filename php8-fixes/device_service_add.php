<?php
	...
	$mfg=new Service();
        ...
        $Machine_list = $Device->GetDevice_newList();
	...
	if(isset($_POST["action"])&&(($_POST["action"]=="Create")||($_POST["action"]=="Update"))){
            ...
            if($mfg->Name != null && $mfg->Name != ""){
                if($_POST["action"]=="Create"){
                    if($mfg->CreateDeviceServiceObject()){
                        header('Service: '.redirect("device_service_add.php?PortID=$mfg->PortID"));
                    }else{
                        $status = __("Error adding new object");
                    }
                } else {
                    $status = __("Updated");
                    $mfg->UpdateDeviceServiceObject();
                }
            }
            //We either just created a manufacturer or updated it so reload from the db
            $mfg->GetOrderByID();    
	}
	$mfgList=$mfg->GetDeviceServiceList();
        $VMList = $Device->GetDevice_newList();
  ...
  <script type="text/javascript">
	$(document).ready(function() {
            ...
            $('#PortID').change(function(e){
                    location.href='device_service_add.php?PortID='+this.value;
            });
