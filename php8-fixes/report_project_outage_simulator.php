...
if (!isset($_REQUEST['action'])) {
	...
	$pan = new PowerPanel();
                                ...
                                foreach ($ids as $id) {
                                    ...
                                    $comp = $status[$statusKeys['Down']] + $status[$statusKeys['Degraded']] + $status[$statusKeys['Degraded/Fail-Safe']];
                                    $sum = $comp + $status[$statusKeys['Normal']] + $status[$statusKeys['Undocumented']];

                                    if ($skipNormal && $comp == 0)
                                        continue;

                                    $activeSheet->SetCellValue('C' . $row, $comp > 0 ? '' . floor($comp * 100. / $sum) . '% ' . __("Compromised") : __("Normal") );
                                    if ($comp > 0)
                                        $activeSheet->getStyle('C' . $row)->applyFromArray(
