<?php
	...
	if(isset($_POST["action"])&&(($_POST["action"]=="Create")||($_POST["action"]=="Update"))){
		...
		$mfg->Name=trim($_POST["name"]);
                ...
                $mfg->Dictionary=trim($_POST["Dictionary"]);
		
		if($mfg->Name != null && $mfg->Name != ""){
			if($_POST["action"]=="Create"){
				if($mfg->CreateObject()){
					header('Location: '.redirect("attributes.php?PortID=$mfg->PortID"));
				}else{
	...
	$mfgList=$mfg->GetAttributesList();
        $mfgObjectList=$mfg->GetDictionaryList();
  ...
  <script type="text/javascript">
	$(document).ready(function() {
		...
		$('#PortID').change(function(e){
			location.href='attributes.php?PortID='+this.value;
		});
...
<div class="form-group">
   ...
   <input type="hidden" name="action" value="query"><select name="Type" id="Type" class="form-control">
    <option value="">-- Type --</option>';
    if($mfg->Type == "Unsigned Integer"){
        $selected1 =" selected";
    } else if($mfg->Type == "Floating Point"){
        $selected2 =" selected";
    } else if($mfg->Type == "String"){
        $selected3 =" selected";
    } else if($mfg->Type == "Dictionary"){
        $selected4 =" selected";
    } else if($mfg->Type == "Date"){
        $selected5 =" selected";
    } else {
        $selected1 = "";
        $selected2 = "";
    ...
    echo '<option value="Unsigned Integer" '.$selected1.'>Unsigned Integer</option>
    <option value="Floating Point" '.$selected2.'>Floating Point</option>
    <option value="String" '.$selected3.'>String</option>
    <option value="Dictionary Record" '.$selected4.'>Dictionary Record</option>
    <option value="Date" '.$selected5.'>Date</option>
   </select>
</div>
<div class="form-group">
   <label for="Dictionary">',__("Dictionary"),'</label>
   <input type="hidden" name="action" value="query"><select name="Dictionary" id="Dictionary" class="form-control">
   <option value=0>',__("-- Dictionary --"),'</option>';

	foreach($mfgObjectList as $mfgRow){
            if($mfg->Dictionary==$mfgRow->PortID){$selected=" selected";}else{$selected="";}
            echo "<option value=\"$mfgRow->PortID\"$selected>$mfgRow->Name</option>\n";
	}
