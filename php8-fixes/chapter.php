<?php
	...
	if(isset($_REQUEST["ChapterID"]) && $_REQUEST["ChapterID"] >0){
		$mfg->ChapterID=(isset($_POST['ChapterID']) ? $_POST['ChapterID'] : $_GET['ChapterID']);
		$mfg->GetOrderByID();
                //print_r($mfg);exit;
	}
	...
	if(isset($_POST["action"])&&(($_POST["action"]=="Create")||($_POST["action"]=="Update"))){
		...
		if($mfg->Name != null && $mfg->Name != ""){
			if($_POST["action"]=="Create"){
				if($mfg->CreateObject()){
					header('Location: '.redirect("chapter.php?ChapterID=$mfg->ChapterID"));
				}else{
...
<head>
  ...
  <script type="text/javascript">
	$(document).ready(function() {
		...
		$('#ChapterID').change(function(e){
			location.href='chapter.php?ChapterID='+this.value;
		});
	...
	function UpdateCount(e){
            ...
            $.ajax({
                    type:'get',
                    async: false, 
                    data:{getTemplateCount: $('#ChapterID').val()},
	...
	function DeleteObject(){
            ...
            $.post('',{ChapterID: $('#ChapterID').val(), TransferTo: transferto, action: 'Delete'},function(data){
                    if(data){
                            location.href='';
                    }else{
