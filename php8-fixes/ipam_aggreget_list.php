...
if($_GET['aggreget']){
    $filter['aggreget'] = $_GET['aggreget'];
}
if($_GET['rir']){
    $filter['rir'] = $_GET['rir'];
}

$aggreget_arr = $aggreget->GetAggreegetListRows($filter);
$aggreget_res = json_decode(json_encode($aggreget_arr), true);

$rir_list_arr = $IpamRIR->GetIpamRIRList();
$rir_list_res = json_decode(json_encode($rir_list_arr), true); 

$aggreget_list_arr = $aggreget->GetIpamAggregetList();
$aggreget_list_res = json_decode(json_encode($aggreget_list_arr), true);

/* echo "<pre>";
print_r($aggreget_res);
echo "<pre/>";exit; */
...
if(isset($_GET['export'])){
    $response = $aggreget->ExportReport($filter);
    exit;
}
// Export Code End
?>
    ...
    <!-- LISTING CODE START -->
        <div class="container wrapper">
            ...
            <div class="row">
                ...
                <div class="col-sm-3">
                    <a href="ipam_aggreget.php" class="btn btn-primary">
                            ...
                            <table class="table table-hover table-headings table-striped">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="orderable"><a href="?sort=a.prefix&sort_by=<?= $sort_by ?>">Aggregate</a></th>
                                        <th class="orderable"><a href="?sort=r.name&sort_by=<?= $sort_by ?>">RIR</a></th>
                                        <th class="orderable"><a href="?sort=a.prefix&sort_by=<?= $sort_by ?>">Prefixes</a></th>
                                        <th class="orderable"><a href="?sort=a.date_added&sort_by=<?= $sort_by ?>">Added</a></th>
                                        <th class="orderable"><a href="?sort=a.description&sort_by=<?= $sort_by ?>">Description</a></th>
                                        <th class="primary_ip">Action</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php if(count($aggreget_res) > 0){
                                        foreach($aggreget_res as $res) { ?>
                                    <tr>
                                        <td><?php echo '<a href="ipam_aggreget_detail.php?id='.$res['PortID'].'">'.$res['Prefix'].'</a>'; ?></td>
                                        <td><?php echo $res['RIRname'] ?></td>
                                        <td><?php echo $res['Prefix'] ?></td>
                                        <td><?php echo $res['DateAdded']!=""?date("m/d/Y",strtotime($res['DateAdded'])):""; ?></td>
                                        <td><?php echo $res['Description'] ?></td>
                                        <td><a href="ipam_aggreget.php?PortID=<?php echo $res['PortID'] ?>"><i class="fa fa-edit"></i></a>
                                ...
                                <tfoot>
                                    <tr><td colspan="6" class="text-right">
                                        <?php 
                                            $limit = 15;
                                            $total_rec = $aggreget->GetDashIpamAggregetList(); 
                                            $total_pages = ceil($total_rec['total_aggreeget'] / $limit); 
                ...
                <div class="col-md-3">
                    <div class="panel panel-default">
                        ...
                        <div class="panel-body">
                            <form action="ipam_aggreget_list.php" method="get" class="form">
                                <div class="form-group">
                                    <label for="">Aggregate</label>
                                    <select name="aggregate" placeholder="Aggregate" size="6" multiple="multiple" id="id_aggregate" class="form-control">
                                        <?php foreach($aggreget_list_res as $val) { ?>
                                        <option value="<?php echo $val['PortID'] ?>" <?php echo $val['PortID']==$_GET['aggregate']?'selected':''?>><?php echo $val['Prefix'] ?></option>
                                        <?php } ?>
                                    </select>
                                </div>
                                <div class="form-group">
                                    <label for="id_rir">RIR</label>
                                    <select name="rir" placeholder="None" size="6" multiple="multiple" id="id_rir" class="form-control">
                                        <?php foreach($rir_list_res as $val) { ?>
                                        <option value="<?php echo $val['PortID'] ?>" <?php echo $val['PortID']==$_GET['rir']?'selected':''?>><?php echo $val['Name'] ?></option>
                                        <?php } ?>
                                    </select>
                                </div>
                                <div class="text-right">
                                    ...
                                    <a href="ipam_aggreget_list.php" class="btn btn-default" style="line-height:1 !important">
