<?php
	...
	if(isset($_POST["action"])&&(($_POST["action"]=="Create")||($_POST["action"]=="Update"))){
            ...
            $mfg->PDU=trim($_POST["pdu"]);
            $mfg->Device=trim($_POST["device"]);
            $mfg->Outlet=trim($_POST["outlet"]);
            $mfg->Power_port=trim($_POST["power_port"]);
            
            if($mfg->Name != null && $mfg->Name != ""){
                    if($_POST["action"]=="Create"){
                            if($mfg->CreateObject()){
                                    header('Location: '.redirect("power_conn.php?PortID=$mfg->PortID"));
                            }else{
  ...
  <script type="text/javascript">
	$(document).ready(function() {
            ...
            $('#PortID').change(function(e){
                    location.href='power_conn.php?PortID='+this.value;
            });
    ...
    <div class="row">
        <div class="col-sm-12 col-md-12">
            <ol class="breadcrumb">
                ...
                <li><a href="power_conn_list.php">Power Connection</a></li>
                <li><?php echo $mfg->PortID!=""?'Edit':'Add';?> Power Connection</li>
            </ol>
        </div>
    </div>
    <!-- Breadcrumb code end -->
...
<div class="panel panel-default">
    <div class="panel-heading"><strong>Power Connection</strong></div>
    ...
    <div class="form-group">
       <label class="col-sm-3" for="pdu">',__("PDU"),'</label>
       <div class="col-sm-9">
       <input type="text" class="form-control" name="pdu" id="pdu" value="',$mfg->PDU,'">
       </div>
    </div>
    <div class="form-group">
    ...
    <div class="form-group">
       <label class="col-sm-3" for="outlet">',__("Outlet"),'</label>
       <div class="col-sm-9">
       <input type="text" class="form-control" name="outlet" id="outlet" value="',$mfg->Outlet,'">
       </div>    
    </div>
    <div class="form-group">
       <label class="col-sm-3" for="power_port">',__("Power Port"),'</label>
       <div class="col-sm-9">
       <input type="text" class="form-control" name="power_port" id="power_port" value="',$mfg->Power_port,'">
       </div>    
    </div>
</div>
</div>
<div class="text-center">';
    ...
    echo '&nbsp;<a href="power_conn_list.php" class="btn-panel btn-success">',__("Cancel"),'</a>';
?>
</div>
    </div>
</div>
