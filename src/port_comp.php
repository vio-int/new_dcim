<?php
	...
	if(isset($_POST["action"])&&(($_POST["action"]=="Create")||($_POST["action"]=="Update"))){
		...
		$mfg->Name=trim($_POST["name"]);
                $mfg->FormInterfaceID=trim($_POST["FormInterfaceID"]);
                $mfg->ToInterfaceID=trim($_POST["ToInterfaceID"]);
		
		if($mfg->Name != null && $mfg->Name != ""){
			if($_POST["action"]=="Create"){
				if($mfg->CreateObject()){
					header('Location: '.redirect("port_comp.php?PortID=$mfg->PortID"));
				}else{
	...
	$mfgList=$mfg->GetPortCompList();
        $mfgObjectList=$mfg->GetPortList();
  ...
  <script type="text/javascript">
	$(document).ready(function() {
		...
		$('#PortID').change(function(e){
			location.href='port_comp.php?PortID='+this.value;
		});
...
<div class="form-group">
   <label for="FormInterfaceID">',__("From Interface"),'</label>
   <input type="hidden" name="action" value="query"><select name="FormInterfaceID" id="FormInterfaceID" class="form-control">
   <option value=0>',__("From Interface"),'</option>';

	foreach($mfgObjectList as $mfgRow){
                if($mfg->FromInterfaceID==$mfgRow->PortID){$selected=" selected";}else{$selected="";}
		echo "<option value=\"$mfgRow->PortID\"$selected>$mfgRow->Name</option>\n";
	}
...
<div class="form-group">
   <label for="ToInterfaceID">',__("To Interface"),'</label>
   <input type="hidden" name="action" value="query"><select name="ToInterfaceID" id="ToInterfaceID" class="form-control">
   <option value=0>',__("To Interface"),'</option>';

	foreach($mfgObjectList as $mfgRow){
                if($mfg->ToInterfaceID==$mfgRow->PortID){$selected=" selected";}else{$selected="";}
		echo "<option value=\"$mfgRow->PortID\"$selected>$mfgRow->Name</option>\n";
	}
