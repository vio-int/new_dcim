...
function startmap(){
	...
	$.ajax({
		...
		type: "post",
		async: false,
		data: {id: $('map[name=datacenter]').data('dc'), getobjects: ''}, 
		success: function(data){
			...
			for(var i in data.cab){
				...
				//if(thiscab.Rights!="None"){
					...
					temp.cabs.push({'CabinetID':thiscab.PortID,'MapX1':thiscab.X1,'MapX2':thiscab.X2,'MapY1':thiscab.Y1,'MapY2':thiscab.Y2});
			...
			for(var i in data.ac){
				...
				temp.acs.push({'PortID':thisac.PortID, 'MapX1':thisac.MapX1,'MapX2':thisac.MapX2,'MapY1':thisac.MapY1,'MapY2':thisac.MapY2});
				temphilight.acs[thisac.PortID]=false;
			};
	...
	$.ajax({
		...
		type: "post",
		async: false,
		data: {id: $('map[name=datacenter]').data('dc'), getoverview: ''}, 
		success: function(d){
			stat=$.extend(true,{},d);
		}
	});
	...
	function buildarea(obj){
		...
		var ac=typeof(obj.PortID)!=='undefined';
		...
		if ( zone ) {
			...
			obj.ZoneID=0;
                        ...
                        var label=obj.Name;
			var name='cab'+obj.PortID;
			var href='rack.php?PortID='+obj.PortID;
			var row=false;
			obj.ZoneID=0;
                        var style = 'background-color:red';
		}else {
                        console.log("Call Builder Else");
		...
		return $('<area>').attr({'shape':'rect','coords':x1+','+y1+','+x2+','+y2,'alt':label,'href':href,'name':name, 'style':style}).data({'hilight':false,'zone':obj.ZoneID,'row':row});
	}
...
function bindmaptooltips(){
	$('map[name="datacenter"]').on('mouseenter','area[name^="cab"],area[name^="ac"]',function(){
