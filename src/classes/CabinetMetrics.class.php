...
class CabinetMetrics {
	...
	static function getMetrics( $CabinetID ) {
		...
		$sql = "select max(Temperature) as Temp, max(Humidity) as Humid, LastRead from fac_densorreadings where DeviceID in (select DeviceID from fac_device where DeviceType='Sensor' and BackSide=1 and Cabinet=:CabinetID)";
		$st = $dbh->prepare( $sql );
		$st->execute( $params );
		if ( $row = $st->fetch() ) {
