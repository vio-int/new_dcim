...
$ipamVLANGroup = new IpamVLANGroup();
$log = new LogActions();
$ipamVlan = new IpamVLAN();

$filter = array();
$filter['group'] = $_GET['id'];

$ipamVLANGroup_arr = $ipamVLANGroup->GetVLANGROUPOne($filter);
$ipamVLANGroup_res = json_decode(json_encode($ipamVLANGroup_arr), true);

$filter_2 = array();
$filter_2['sort_by'] = "Asc";
$filter_2['sort_on'] = "v.id";
$filter_2['group'] = $ipamVLANGroup_res[0]['PortID'];
$ipamVLAN_arr = $ipamVlan->GetIpamVLANListRows($filter_2);
$ipamVLAN_res = json_decode(json_encode($ipamVLAN_arr), true);

//print_r($ipamVLANGroup_res);exit;
$filter_2 = array();
$filter_2['sort_on'] = "Time";
$filter_2['sort_by'] = "DESC";
$filter_2['class'] = "IpamVLANGroup";
$filter_2['object'] = $ipamVLANGroup_res[0]['PortID'];
...
if (isset($_POST['action']) && $_POST["action"] == "Delete") {
    ...
    if (isset($_POST["TransferTo"])) {
        $ipamVLANGroup->PortID = $_POST['PortID'];
        if ($ipamVLANGroup->DeleteObject($_POST["TransferTo"])) {
            $response = true;
        }
    }
                ...
                <div class="row">
                    <div class="col-sm-12 col-md-12">
                        <ol class="breadcrumb">
                            ...
                            <li><a href="ipam_vlan_group_list.php">VLAN Group</a></li>
                            <li><?php echo $ipamVLANGroup_res[0]['Name']; ?></li>
                        </ol>
                    </div>
                </div>
                <div class="pull-right">
                    <a href="ipam_vlan_group.php?PortID=<?php echo $ipamVLANGroup_res[0]['PortID'] ?>" class="btn btn-warning">
                        <span class="fa fa-edit" aria-hidden="true"></span>
                        Edit this VLAN Group
                    </a>
                    <a href="javascript:void(0);" class="btn btn-danger" id="delete" data-id="<?php echo $ipamVLANGroup_res[0]['PortID'] ?>">
                        <span class="fa fa-trash" aria-hidden="true"></span>
                        Delete this VLAN Group
                    </a>
                </div>
                <h1><?php echo $ipamVLANGroup_res[0]['Name']; ?></h1>
                <p>
                    <small class="text-muted">Created <?php echo $ipamVLANGroup_res[0]['Created_at']!="0000-00-00"?date("M. d, Y", strtotime($ipamVLANGroup_res[0]['Created_at'])):"-"; ?> &middot; Updated <?php echo $ipamVLANGroup_res[0]['Updated_at']!="0000-00-00"?date("M. d, Y", strtotime($ipamVLANGroup_res[0]['Updated_at'])):"-"; ?></small>
                </p>
                <ul class="nav nav-tabs" style="margin-bottom: 20px">
                    <li class="tablinks active" onclick="openTab(event, 'VLAN')" >
                        <a href="javascript:void(0);">VLAN Group</a>
                    </li>
                        ...
                        <div class="col-md-12">
                            <div class="panel panel-default">
                                ...
                                <table class="table table-hover table-headings table-striped">
                                    ...
                                    <?php if(count($ipamVLAN_res) > 0){
                                        foreach($ipamVLAN_res as $val){?>
                                        <tr>
                                    ...
                                    <tfoot>
                                        <tr><td colspan="6" class="text-right">
                                            <?php 
                                                ...
                                                $pagLink = "<ul class='pagination'><li class='page-item page_go' data-id='1' data-obj='".$ipamVLANGroup_res[0]['PortID']."'><a class='page-link' href='javascript:void(0);'>&laquo;</a></li>"; 
                                                for ($i=1; $i<=$total_pages; $i++) {
                                                    ...
                                                    $pagLink .= "<li class='page-item page_go' data-id='".$i."' data-obj='".$ipamVLANGroup_res[0]['PortID']."'><a class='page-link ".$selected."' href='javascript:void(0);'>".$i."</a></li>";	
                                                }
                                                echo $pagLink . "<li class='page-item page_go' data-id='".$total_pages."' data-obj='".$ipamVLANGroup_res[0]['PortID']."'><a class='page-link' href='javascript:void(0);'>&raquo;</a></li></ul>"; 
                                            ?>
                                        </td></tr>
                                    </tfoot>
                                    <?php } */ ?>    
    ...
    $(document).ready(function () {
        $('#delete').click(function () {
            if (confirm('Are you sure you want to delete this record ?')) {
                ...
                $.post('', {PortID: $(this).data("id"), TransferTo: transferto, action: 'Delete'}, function (data) {
                    if (data) {
                        location.href = 'ipam_vlan_group_list.php';
                    } else {
        ...
        $('.page_go').click(function () {
            ...
            $.ajax({
                ...
                data: {page:page_no, class: "IpamVLANGroup", object:obj_id},
                dataType: 'JSON',
