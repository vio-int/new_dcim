...
if (isset($_POST["action"]) && (($_POST["action"] == "Create") || ($_POST["action"] == "Update"))) {
    ...
    if ($mfg->Name != null && $mfg->Name != "") {
        if ($_POST["action"] == "Create") {
            if ($mfg->CreateObject()) {
                header('Location: ' . redirect("port_status.php?PortID=$mfg->PortID"));
            } else {
        ...
        <script type="text/javascript">
            $(document).ready(function () {
                ...
                $('#PortID').change(function (e) {
                    location.href = 'port_status.php?PortID=' + this.value;
                });
...
<div class="form-group">
   <label for="Status">', __("Status"), '</label>';
                    ...
                    if ($mfg->Status == "Inactive") {
                        $selected1 = " selected";
                    } else {
                        $selected1 = "";
                    }
                    echo '<input type="hidden" name="action" value="query"><select name="Status" id="Status" class="form-control">
   <option value=0>', __("-- Select --"), '</option>
   <option value=Active '.$selected.'>', __("Active"), '</option>
   <option value=Inactive '.$selected1.'>', __("Inactive"), '</option>';
                    echo '</select>
</div>
