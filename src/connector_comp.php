<?php
	...
	if(isset($_POST["action"])&&(($_POST["action"]=="Create")||($_POST["action"]=="Update"))){
            ...
            $mfg->Cable_type=trim($_POST["Cable_type"]);
            $mfg->Connector=trim($_POST["Connector"]);

            if($mfg->Name != null && $mfg->Name != ""){
                    if($_POST["action"]=="Create"){
                            if($mfg->CreateObject()){
                                    header('Location: '.redirect("connector_comp.php?PortID=$mfg->PortID"));
                            }else{
	...
	$mfgList=$mfg->GetConnectorCompList();
        $mfgConnectorList = $mfg->GetConnectorList();
        $mfgCableList = $mfg->GetCableList();
  ...
  <script type="text/javascript">
	$(document).ready(function() {
            ...
            $('#PortID').change(function(e){
                    location.href='connector_comp.php?PortID='+this.value;
            });
...
<div class="form-group">
   <label for="PortID">',__("Connector Compatibility"),'</label>
   <input type="hidden" name="action" value="query"><select name="PortID" id="PortID" class="form-control">
   <option value=0>',__("New Connector Compatibility"),'</option>';
...
<div class="form-group">
   <label for="Connector">',__("Connector"),'</label>
   <input type="hidden" name="action" value="query"><select name="Connector" id="Connector" class="form-control">
   <option value=0>',__("-- Connector --"),'</option>';

	foreach($mfgConnectorList as $mfgRow){
            if($mfg->Connector==$mfgRow->PortID){$selected=" selected";}else{$selected="";}
            echo "<option value=\"$mfgRow->PortID\"$selected>$mfgRow->Connector</option>\n";
	}

echo '</select>
</div>
<div class="form-group">
   <label for="Cable Type">',__("Cable Type"),'</label>
   <input type="hidden" name="action" value="query"><select name="Cable_type" id="Cable_type" class="form-control">
   <option value=0>',__("-- Cable_type --"),'</option>';

	foreach($mfgCableList as $mfgRow){
            if($mfg->Cable_type==$mfgRow->PortID){$selected=" selected";}else{$selected="";}
            echo "<option value=\"$mfgRow->PortID\"$selected>$mfgRow->Cable_type</option>\n";
	}
