...
if($_GET['device_a']){
    $filter['device_a'] = $_GET['device_a'];
}
if($_GET['device_b']){
    $filter['device_b'] = $_GET['device_b'];
}

$InterfaceConn_arr = $InterfaceConn->GetInterfaceConnListRows($filter);
$InterfaceConn_res = json_decode(json_encode($InterfaceConn_arr), true);

$Device_list_arr = $Device->GetDevice_newList();
$Device_list_res = json_decode(json_encode($Device_list_arr), true);

/* echo "<pre>";
print_r($InterfaceConn_res);
echo "<pre/>";exit; */
// Delete Code Start
if(isset($_POST['action']) && $_POST["action"]=="Delete"){
    ...
    if(isset($_POST["TransferTo"])){
        $InterfaceConn->PortID=$_POST['PortID'];
        if($InterfaceConn->DeleteObject($_POST["TransferTo"])){
            $response=true;
        }
    }
...
if(isset($_GET['export'])){
    $response = $InterfaceConn->ExportReport($filter);
    exit;
}
// Export Code End
?>
    ...
    <!-- LISTING CODE START -->
        <div class="container wrapper">
            ...
            <div class="row">
                ...
                <div class="col-sm-3">
                    <a href="interface_conn.php" class="btn btn-primary">
                            ...
                            <table class="table table-hover table-headings table-striped">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="orderable"><a href="?sort=v.device_a&sort_by=<?= $sort_by ?>">Device A</a></th>
                                        <th class="orderable"><a href="?sort=v.interface_a&sort_by=<?= $sort_by ?>">Interface A</a></th>
                                        <th class="orderable"><a href="?sort=v.device_b&sort_by=<?= $sort_by ?>">Device B</a></th>
                                        <th class="orderable"><a href="?sort=v.interface_b&sort_by=<?= $sort_by ?>">Interface B</a></th>
                                        <th class="primary_ip">Action</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php if(count($InterfaceConn_res) > 0){
                                        foreach($InterfaceConn_res as $res) { ?>
                                    <tr>
                                        <td><?php echo $res['Device_name_a'] ?></td>
                                        <td><?php echo $res['Interface_a'] ?></td>
                                        <td><?php echo $res['Device_name_b'] ?></td>
                                        <td><?php echo $res['Interface_b'] ?></td>
                                        <td>
                                            <a href="interface_conn.php?PortID=<?php echo $res['PortID'] ?>"><i class="fa fa-edit"></i></a>
                                ...
                                <tfoot>
                                    <tr><td colspan="5" class="text-right">
                                        <?php 
                                            ...
                                            $total_rec = $InterfaceConn->GetDashInterfaceList(); 
                                            $total_pages = ceil($total_rec['total_interface'] / $limit); 
                ...
                <div class="col-md-3">
                    <div class="panel panel-default">
                        ...
                        <div class="panel-body">
                            <form action="interface_conn_list.php" method="get" class="form">
                                <div class="form-group">
                                    <label for="">Device A</label>
                                    <select name="device_a" id="device_a" class="form-control">
                                        <option value="">-- Select --</option>
                                        <?php foreach($Device_list_res as $val) { ?>
                                        <option value="<?php echo $val['PortID'] ?>" <?php echo $val['PortID']==$_GET['device_a']?'selected':''?>><?php echo $val['Name'] ?></option>
                                        <?php } ?>
                                    </select>
                                </div>
                                <div class="form-group">
                                    <label for="">Device B</label>
                                    <select name="device_b" id="device_b" class="form-control">
                                        <option value="">-- Select --</option>
                                        <?php foreach($Device_list_res as $val) { ?>
                                        <option value="<?php echo $val['PortID'] ?>" <?php echo $val['PortID']==$_GET['device_b']?'selected':''?>><?php echo $val['Name'] ?></option>
                                        <?php } ?>
                                    </select>
                                </div>
                                <div class="text-right">
                                    ...
                                    <a href="interface_conn_list.php" class="btn btn-default" style="line-height:1 !important">
