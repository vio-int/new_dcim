<?php
	...
	if(isset($_REQUEST["MainID"]) && $_REQUEST["MainID"] >0){
            ...
            $mfgIPVList = $mfg->GetIPv4ID($mfg->IpID);
            //print_r($mfg);exit;
	}
        
        if(isset($_REQUEST["IpID"]) && $_REQUEST["IpID"] >0){
            $mfg->IpID=(isset($_POST['IpID']) ? $_POST['IpID'] : $_GET['IpID']);
            $mfgIPVList = $mfg->GetIPv4ID($_REQUEST["IpID"]);
        } else {
            $mfgIPVList = $mfg->GetIPv4ID();
        }
	...
	if(isset($_POST["action"])&&(($_POST["action"]=="Create")||($_POST["action"]=="Update"))){
            ...
            if($mfg->Name != null && $mfg->Name != ""){
                    if($_POST["action"]=="Create"){
                            if($mfg->CreateObject()){
                                    header('Location: '.redirect("ipv4allocation.php?MainID=$mfg->MainID"));
                            }else{
	...
	$mfgList=$mfg->GetIPv4AllocationList();
  ...
  <script type="text/javascript">
	$(document).ready(function() {
            ...
            $('#MainID').change(function(e){
                    location.href='ipv4allocation.php?MainID='+this.value;
            });
...
<div>
   <div><label for="IpID">',__("IPv4"),'</label></div>
