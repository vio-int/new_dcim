...
$ClusterType_arr = $ClusterType->GetClusterTypeListRows($filter);
$ClusterType_res = json_decode(json_encode($ClusterType_arr), true);


/* echo "<pre>";
print_r($ClusterType_res);
echo "<pre/>";exit; */
...
if(isset($_POST['action']) && $_POST["action"]=="Delete"){
    ...
    if(isset($_POST["TransferTo"])){
        $ClusterType->PortID=$_POST['PortID'];
        if($ClusterType->DeleteObject($_POST["TransferTo"])){
            $response=true;
        }
    }
...
if(isset($_GET['export'])){
    $response = $ClusterType->ExportReport($filter);
    exit;
}
// Export Code End
?>
    ...
    <!-- LISTING CODE START -->
        <div class="container wrapper">
            ...
            <div class="row">
                ...
                <div class="col-sm-3">
                    <a href="cluster_type.php" class="btn btn-primary">
                ...
                <div class="col-md-12">
                    <div class="table-responsive">
                            <table class="table table-hover table-headings table-striped">
                                <thead>
                                    <tr>
                                        <th class="orderable"><a href="?sort=v.name&sort_by=<?= $sort_by ?>">Type Name</a></th>
                                        <th class="orderable"><a href="?sort=v.slug&sort_by=<?= $sort_by ?>">Slug</a></th>
                                        <th class="orderable">Clusters</th>
                                        <th class="primary_ip">Action</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php if(count($ClusterType_res) > 0){
                                        foreach($ClusterType_res as $res) { ?>
                                    <tr>
                                        <td><?php echo '<a href="cluster_list.php?type='.$res['PortID'].'">'.$res['Name'].'</a>'; ?></td>
                                        <td><?php echo $res['Slug'] ?></td>
                                        <td><?php echo $res['Total_cluster']!=""?$res['Total_cluster']:"0"; ?></td>
                                        <td><a href="cluster_type.php?PortID=<?php echo $res['PortID'] ?>"><i class="fa fa-edit"></i></a>
                                        <a href="javascript:void(0);" class="delete" data-id="<?php echo $res['PortID'] ?>"><i class="fa fa-trash"></i></a></td>
                                    </tr>    
                                    <?php } } else { ?>
                                    <tr class="text-center"><td colspan="4">Record not found!</td></tr> 
                                    <?php } ?>
                                </tbody>
                                <?php if(count($ClusterType_res) > 0){ ?>
                                <tfoot>
                                    <tr><td colspan="4" class="text-right">
                                        <?php 
                                            ...
                                            $total_rec = $ClusterType->GetDashClusterTypeList(); 
                                            $total_pages = ceil($total_rec['total_type'] / $limit); 
